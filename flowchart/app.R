                   ...
                   screening were \'combined\' or performed \'separately\'.',
                         ...
                         tabPanel("Create flow chart",
                                  column(12,
                                         ...
                                         column(3,
                                                selectInput("include_prescreened_input", "Include prescreened records:", choices=c('yes', 'no')),
                                                selectInput("synthesis_type", "Select type of synthesis:", choices=c('narrative', 'quantitative', 'qualitative', 'mixed'))),
                                         column(6,
                                                tags$a(href="https://www.roses-reporting.com/", tags$img(height = "100px", src = "https://github.com/nealhaddaway/ROSES_flowchart/blob/master/inst/extdata/roses.png?raw=true")))
                                  ),
                                  sidebarLayout(  
                                      
                                      sidebarPanel(style = "overflow-y:scroll; max-height: 900px; position:relative;",
                                                   h3("Data sources"),
                                                   splitLayout(textInput("dbresults", label = 'Database results:', value = '789'),
                                                   ...
                                                   conditionalPanel(
                                                       ...
                                                       splitLayout(textInput("caincl", label = "Studies included after CA:", value = ''),
                                                                   textInput("caexcl", label = "Studies excluded after CA:", value = '', placeholder = 'Reason1, n; Reason2, n; Reason3, n')),
                                                       ...
                                                       splitLayout(textInput("finalincl", label = "Final included studies:", value = ''),
                                                                   textInput("finalexcl", label = "Studies not included:", value = '', placeholder = 'Reason1, n; Reason2, n; Reason3, n'))
...
server <- function(input, output) {
    ...
    plot <- reactive({
        ...
        if(input$include_prescreened_input == 'yes'){
            prescreenedinput <- TRUE
        } else {
            prescreenedinput <- FALSE
        }
        ...
        caexcl <- data.frame(reason = gsub(",.*$", "", unlist(strsplit(input$caexcl, split = '; '))), 
                             n = gsub(".*,", "", unlist(strsplit(input$caexcl, split = '; '))))
        finalexcl <- data.frame(reason = gsub(",.*$", "", unlist(strsplit(input$finalexcl, split = '; '))), 
                                n = gsub(".*,", "", unlist(strsplit(input$finalexcl, split = '; '))))
        ...
        plot <- ROSES_flowchart(dbresults = input$dbresults,
                                ...
                                include_prescreened = prescreenedinput,
